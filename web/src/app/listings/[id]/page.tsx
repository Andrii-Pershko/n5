...
export default function AssetDetailPage() {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
      <article>
        <div className="flex flex-wrap gap-2 text-[11px] uppercase tracking-[0.16em] text-gold">
          {asset.isTopDeal && <span>{t('asset.topDeal')}</span>}
          {asset.isValidated && (
            <span className="rounded-full border border-gold/40 px-2 py-0.5">{t('asset.validated')}</span>
          )}
        </div>
        <h1 className="mt-3 text-3xl">Asset ID #{asset.publicCode.replace('ND-', '')}</h1>
        <p className="mt-2 text-muted">{asset.title}</p>
        <p className="mt-6 text-3xl text-gold">{formatPrice(asset.priceEur)}</p>
        ...
        <p className="mt-6 border border-line bg-card p-4 text-xs leading-6 text-muted">
          {t('asset.disclaimer')}
        </p>
      </article>
